...
2. Cursor will open a configuration screen pre-filled with:
   ```jsonc
   {
     ...
     "env": {
       ...
       // Optional strict additions layered over a base map that cannot be edited:
       "PROPERTY_QUERY_TABLE_MAP_ADDITIONS": "{\"rock-island\":\"https://ipfs.filebase.io/ipns/k51…\"}",
       // Public City of Rock Island issued-permit report query table:
       "PERMIT_QUERY_TABLE_MAP_ADDITIONS": "{\"rock-island\":\"https://ipfs.filebase.io/ipns/k51qzi5uqu5di42nblo5nuk94aj7af393d9y5vhqxp5dtxikzso0wt14v3p0wa\"}",
       // Public organization registry: stable manifest IPNS, resolved to its
       // current immutable Parquet CID after exact privacy/schema validation:
       "CORPORATE_REGISTRATION_MANIFEST_MAP_ADDITIONS": "{\"rock-island\":\"https://ipfs.filebase.io/ipns/k51qzi5uqu5dggdjnm0bym1p10gtu3o7dz3ipfaa4ccxgs73oohaqndzegk16d\"}",
       // Optional override/addition: per-county hourly coverage snapshots.
       // Lee, Miami-Dade, Orange, Palm Beach, and Rock Island coverage URLs are built in.
       "DATASET_COVERAGE_MAP": "{\"lee\":\"https://ipfs.filebase.io/ipns/<coverage-ipns-name>\"}",
       // Strict public-URL additions have highest precedence without replacing the base map:
       "DATASET_COVERAGE_MAP_ADDITIONS": "{\"rock-island\":\"https://elephant-mcp-two.vercel.app/coverage/rock-island.json\"}",
...
`CORPORATE_REGISTRATION_MANIFEST_MAP` and its strict additive layer, `CORPORATE_REGISTRATION_MANIFEST_MAP_ADDITIONS`, map an allowlisted county to a stable public manifest IPNS URL. `queryCorporations` and `getCorporateQuerySchema` validate the exact organization-only manifest, resolve its current Parquet by immutable CID, reject any Parquet schema drift, materialize only the approved columns, and disable DuckDB external access before caller SQL runs. Rock Island is currently the only corporate county allowlisted. Its county scope is the registered-agent office county, not a business operating location, tenancy, occupancy, ownership, or property association/linkage.

`PERMIT_QUERY_TABLE_MAP_ADDITIONS` is the strict additive layer for permit Parquet tables. It preserves Santa Clara and every base-map entry, accepts only absolute HTTP(S) URLs, and fails on conflicting duplicates. Rock Island is locked to the exact 11-column public schema, materialized before DuckDB external access is disabled, and queried only through the `permits` view or CTEs derived from it. Its 47,385 rows contain 24,786 City of Rock Island records from 112 official reports and 22,599 Moline records from 102 supported official reports; 61 ambiguous Moline reports remain excluded. This is not full permit lifecycle/history or countywide coverage; no addresses, parcels, descriptions, contractors, valuations, contacts, or property links are published.

For Rock Island, merge these entries into the deployment's existing JSON maps.
Do not replace other county entries or change any configured default county:

```text
ORACLE_OPEN_DATA_IPNS_MAP={"rock-island":"k51qzi5uqu5dkmvvxp9idpc5x0x1pd1sv901htkwk30j496kik3c9619n2qmp1"}
ORACLE_OPEN_DATA_INDEX_CID_MAP_ADDITIONS={"rock-island":"QmWo6htg7j51ue7BhubgRytVDTgEkAAUffJFZB7GkM9iP4"}
PROPERTY_QUERY_TABLE_MAP_ADDITIONS={"rock-island":"https://ipfs.filebase.io/ipns/k51qzi5uqu5djbtswq6lb4p7xbf3nu8bzdzokdtcdld1r2vx6asn7lgfuk54wt"}
PROPERTY_QUERY_TABLE_CID_FALLBACK_MAP_ADDITIONS={"rock-island":"QmdQ6gd7pvbEPBaMtASiHQtNrW7iGYhByVxAQJrGufT4QG"}
PERMIT_QUERY_TABLE_MAP_ADDITIONS={"rock-island":"https://ipfs.filebase.io/ipns/k51qzi5uqu5di42nblo5nuk94aj7af393d9y5vhqxp5dtxikzso0wt14v3p0wa"}
PERMIT_QUERY_TABLE_CID_FALLBACK_MAP_ADDITIONS={"rock-island":"QmYfhGF427Yvbv8B2e8rvP2idTQp5yEyKCgbj4bzRHGEaW"}
CORPORATE_REGISTRATION_MANIFEST_MAP_ADDITIONS={"rock-island":"https://ipfs.filebase.io/ipns/k51qzi5uqu5dggdjnm0bym1p10gtu3o7dz3ipfaa4ccxgs73oohaqndzegk16d"}
DATASET_COVERAGE_CID_FALLBACK_MAP_ADDITIONS={"rock-island":"QmYbKVaD44u51w8Bf1KUcqnTCr4mDWNRANCW9k4VcPZ317"}
```

The Rock Island coverage IPNS URL is built in. The immutable fallback is scoped
only to Rock Island; unrelated county coverage entries remain untouched.
...
| `PROPERTY_QUERY_TABLE_MAP`                                     | **Recommended for SQL queries.** JSON object mapping county → query-table Parquet location (an IPNS gateway URL or a local path), e.g. `{"lee":"https://ipfs.filebase.io/ipns/k51…"}`. County keys are lowercased and hyphenated (`palm-beach`, not `palm_beach`). Only query-table tools initialize DuckDB; open-data and coverage tools use their IPNS/index sources independently.                                                                                                                                                                                              | _(optional)_              |
| `PROPERTY_QUERY_TABLE_MAP_ADDITIONS`                           | Optional strict JSON object merged into `PROPERTY_QUERY_TABLE_MAP`. Values must be absolute HTTP(S) URLs. Existing base counties are preserved; same-value duplicates are accepted and conflicting duplicates fail closed.                                                                                                                                                                                                                                                                                                                                                         | _(optional)_              |
| `PROPERTY_QUERY_TABLE_CID_FALLBACK_MAP_ADDITIONS`              | Temporary reviewed county→CIDv0 map for stale IPNS gateways. The base property route must remain a stable IPNS URL; the server resolves it first, then reads the configured immutable CID for that county so stale gateway bytes cannot be served. Remove an entry after gateways converge.                                                                                                                                                                                                                                                                                           | _(optional)_              |
...
| `PERMIT_QUERY_TABLE_MAP`                                       | JSON object mapping county → permit query-table Parquet location.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | _(optional)_              |
| `PERMIT_QUERY_TABLE_MAP_ADDITIONS`                             | Strict additive permit map. Absolute HTTP(S) URLs only; preserves base counties such as Santa Clara, accepts same-value duplicates, and fails on conflicts. Rock Island receives exact-schema validation and engine-level external-access denial.                                                                                                                                                                                                                                                                                                                                      | _(optional)_              |
| `PERMIT_QUERY_TABLE_CID_FALLBACK_MAP_ADDITIONS`                | Temporary reviewed county→CIDv0 map for stale permit IPNS gateways. The base permit route remains stable IPNS and is resolved first; only that county may read its immutable fallback.                                                                                                                                                                                                                                                                                                                                                                                               | _(optional)_              |
| `PERMIT_QUERY_TABLE`                                           | Legacy single-county permit query-table location.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | _(optional)_              |
| `PERMIT_QUERY_TABLE_DEFAULT_COUNTY`                            | County served by the legacy single permit query table.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | _(optional)_              |
| `CORPORATE_REGISTRATION_MANIFEST_MAP`                          | JSON object mapping an allowlisted county to its stable public corporate manifest IPNS URL. The manifest is validated exactly and resolves the current immutable Parquet CID.                                                                                                                                                                                                                                                                                                                                                                                                      | _(optional)_              |
| `CORPORATE_REGISTRATION_MANIFEST_MAP_ADDITIONS`                | Strict additive corporate manifest map. Absolute HTTP(S) URLs only; preserves base entries, accepts same-value duplicates, and fails on conflicts. Use this when a deployment's base map is sensitive or unreadable.                                                                                                                                                                                                                                                                                                                                                               | _(optional)_              |
| `CORPORATE_REGISTRATION_MANIFEST`                              | Legacy single corporate manifest location. The caller must still name an allowlisted county.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | _(optional)_              |
| `CORPORATE_REGISTRATION_DEFAULT_COUNTY`                        | County served by the legacy single corporate manifest.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | _(optional)_              |
| `DATASET_COVERAGE_MAP`                                         | Optional JSON object mapping county → published `dataset-coverage.json` location (a Filebase/IPNS gateway URL or a local path for development), e.g. `{"lee":"https://ipfs.filebase.io/ipns/k51…"}`. Lee, Miami-Dade, Orange, Palm Beach, and Rock Island are built in; entries override or add counties. `getOracleDatasetInfo` returns `datasets[]` with per-source (appraisal/permits/corporate/BBB) counts and load timestamps. Corporate entries explicitly retain registered-agent-office county semantics. Coverage is additive — a read failure never breaks dataset-info. | _(optional)_              |
| `DATASET_COVERAGE_MAP_ADDITIONS`                               | Strict JSON object of absolute public HTTP(S) coverage URLs with highest precedence. Use it to add or supersede one county snapshot when the deployment's base coverage map must remain unread and untouched; all unrelated base entries are preserved.                                                                                                                                                                                                                                                                                                                           | _(optional)_              |
| `DATASET_COVERAGE_CID_FALLBACK_MAP_ADDITIONS`                  | Temporary reviewed county→CIDv0 map for stale coverage IPNS gateways. The base coverage route must remain stable IPNS; the server resolves it first, then reads only that county's immutable CID.                                                                                                                                                                                                                                                                                                                       | _(optional)_              |
...
| `PUBLISHED_COUNTY_CATALOG_URL`                                 | Canonical Oracle-published county catalog used by `listPublishedCounties` and as the only source of `queryPlaces` parquet URLs. Accepts an HTTP(S) URL or a local JSON path for development. Places queries require each non-null `placesTableUrl` to use an approved HTTPS IPFS gateway and end in `/places-table.parquet`. Defaults to `oracle-node/main/catalog/published-counties.json`.                                                                                                                                                                                             | Oracle repository catalog |
...
| `ORACLE_OPEN_DATA_INDEX_CID_MAP_ADDITIONS`                     | Temporary reviewed county→index CIDv0 map for stale IPNS gateways. Stable county IPNS is still resolved first; a mismatched or unavailable resolution uses only that county's immutable fallback. Remove the entry after gateway convergence.                                                                                                                                                                                                                                                                                                                                       | _(optional)_              |
