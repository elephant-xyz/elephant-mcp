...
function resolveClass(
    ...
    const classKeys = entries.filter(([, v]) => v.type === "class").map(([k]) => k);

    const target = classKeys.find((k) => normalizeKey(k) === normalizeKey(className));
    if (!target) {
...
export async function listPropertiesByClassNameHandler(className: string) {
    try {
        ...
        const addProps = (node: any) => {
            if (node && typeof node === "object" && node.properties && typeof node.properties === "object") {
                for (const key of Object.keys(node.properties)) {
                    ...
                    const desc = prop && typeof prop === "object" ? (prop as any).description ?? null : null;
        ...
        return createTextResult({ error: "Internal error while listing properties" });
    }
}
